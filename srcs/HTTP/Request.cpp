...
void  Request::set_request(Config conf)
{
  _config = conf;
}
//Destructor
...
int Request::headers()
{
  ...
  if ((_request_headers.find("host") == _request_headers.end() && _request_headers.find("Host") == _request_headers.end() )  || (_request_headers["host"].empty() && _request_headers["Host"].empty()))
    return 400;
